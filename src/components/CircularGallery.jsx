...
class Title {
  ...
  updateLayout() {
    ...
    const aspect = this.mesh.scale.x / Math.max(this.mesh.scale.y, 0.0001);
    const parentScaleX = Math.max(this.plane.scale.x, 0.0001);
    const parentScaleY = Math.max(this.plane.scale.y, 0.0001);
    const localHeight = 0.15;
    const worldHeight = parentScaleY * localHeight;
    const worldWidth = worldHeight * aspect;
    // Compensate for the portrait plane's non-uniform scale so canvas text
    // keeps its natural proportions instead of being stretched horizontally.
    this.mesh.scale.set(worldWidth / parentScaleX, worldHeight / parentScaleY, 1);
    this.mesh.position.y = -0.5 - localHeight * 0.5 - 0.05 / parentScaleY;
  }
}
...
class Media {
  ...
  update(scroll, direction) {
    ...
    this.plane.position.y += this.verticalOffset;
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.y = (this.viewport.height * (1280 * this.scale)) / this.screen.height;
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font, showTitles, verticalOffset) {
    const defaultItems = [
  ...
  addEventListeners() {
    ...
    if (this.enableKeyboard) this.container.addEventListener('keydown', this.boundOnKeyDown);
  }
  destroy() {
    ...
    if (this.enableKeyboard) this.container.removeEventListener('keydown', this.boundOnKeyDown);
  }
}
